...
def test_concurrent_tag_creation_500_is_retried(monkeypatch):
    # Concurrent uploads racing to auto-create the same new tag make szurubooru
    # answer with a bare 500 (UniqueViolation on tag_name); the rolled-back
    # request succeeds on retry (#90)
    monkeypatch.setattr(szurubooru.time, 'sleep', lambda _: None)
    responses = [
    ...
    client = RecordingClient(lambda request: responses.pop(0))
    assert client.szuru.create_post({'tags': ['applying_manicure'], 'contentToken': 'token'}) == 42
    assert len(client.requests) == 2
